...
# Assumption: horizontal axis of view is parallel to xy plane
...
def get_camera_rays(c_pos, width=320, height=240, focal_length=0.035, sensor_width=0.032, noisy=False,
    ...
    c_dir = (track_point - c_pos) # view direction
    ...
    img_plane_horizontal = np.cross(c_dir, vertical) # horizontal axis 
...
def get_rays(c_pos, front, right, up, width=320, height=240, focal_length=0.035, sensor_width=0.032, noisy=False):
    
    img_plane_center = c_pos + front * focal_length
    ...
    horizontal_offsets = np.repeat(np.reshape(
        horizontal_offsets, (1, width)), height, 0)
    vertical_offsets = np.repeat(np.reshape(
        vertical_offsets, (height, 1)), width, 1)

    if noisy:
        pixel_width = sensor_width / width
        pixel_height = sensor_height / height
        horizontal_offsets += (np.random.random((height,
                               width)) - 0.5) * pixel_width
        vertical_offsets += (np.random.random((height, width)
                                              ) - 0.5) * pixel_height

    horizontal_offsets = (np.reshape(horizontal_offsets, (height, width, 1)) *
                          np.reshape(right, (1, 1, 3)))
    vertical_offsets = (np.reshape(vertical_offsets, (height, width, 1)) *
                        np.reshape(up, (1, 1, 3)))
...
def get_vertical_rays(width=320, height=240, focal_length=0.035, sensor_width=0.032, noisy=False):
    # Casting rays in the vertical direction
    c_pos = np.array((-1, 0, 0))
    return get_camera_rays(c_pos, width, height, focal_length, sensor_width, noisy)
